...
HdtDocumentPrototype.searchTriples = function (subject, predicate, object, options) {
  ...
  const offset = options && options.offset ? Math.max(0, parseInt(options.offset, 10)) : 0,
      isLimited = options && !isNaN(options.limit),
      isZeroLimited = isLimited && options.limit === 0,
      // HACK: Set limit to 1 when limit is 0 to differentiate with no limit
      limit = isZeroLimited ? 1 : (isLimited ? Math.max(0, parseInt(options.limit,  10)) : 0);
  ...
  return new Promise((resolve, reject) => {
    this._searchTriples(subject, predicate, object, offset, limit, function (err, triples, totalCount, hasExactCount) {
      if (err)
        ...
        resolve({ triples: isZeroLimited ? [] : triples, totalCount, hasExactCount });
    });
  });
};
  ...
  return new Promise((resolve, reject) => {
    this._searchLiterals(substring, offset, limit, function (err, literals, totalCount) {
      if (err)
        ...
        resolve({ literals: isZeroLimited ? [] : literals, totalCount  });
    });
  });
};
