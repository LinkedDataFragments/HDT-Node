...
HdtDocumentPrototype.searchTerms = function (options) {
  ...
  if (options.filter && (!filterVal || filterVal === ''))
    return Promise.reject(new Error('Unspecified filter value.'));
  if (options.filter && (filterPos !== 'object' || position !== 'predicate'))
    return Promise.reject(new Error('Invalid filter position argument.'));
  ...
  return new Promise((resolve, reject) => {
    if (!options.filter) {
      ...
      this._fetchDistinctTerms(filterVal, filterPosId, limit, posId, (error, results) => {
        if (error)
          reject(error);
        else
          resolve(results);
