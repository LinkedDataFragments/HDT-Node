...
HdtDocumentPrototype.searchTriples = function (subject, predicate, object, options) {
  ...
  options = options || {};
  const offset = isNaN(options.offset) ? 0 : Math.max(0, parseInt(options.offset, 10)),
      limit = isNaN(options.limit) ? MAX : Math.max(0, parseInt(options.limit, 10));
...
HdtDocumentPrototype.searchTerms = function (options) {
  if (this.closed) return Promise.reject(new Error('The HDT document cannot be read because it is closed'));
  options = options || {};
  const limit = isNaN(options.limit) ? MAX : Math.max(0, parseInt(options.limit, 10)),
