...
module.exports = {
  ...
  generateFile: function(rdfFile, targetHdtFile, config, callback, self) {
    ...
    if (config.hasOwnProperty('baseUri') && typeof config.baseUri !== 'string') {
      return callback.call(self, Error('Invalid base URI: ' + config.baseUri));
    }
    ...
    // Construct the native HdtDocument
    hdtNative.rdf2hdt(rdfFile, targetHdtFile, config.hdtSpecs, config.format,function (error) {
      if (error) {
        switch (error.message) {
